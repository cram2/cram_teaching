...
(defun init ()
  ...
  (let ((trash
          `((trash1 . ,(cl-tf:make-pose
                        (cl-tf:make-3d-vector 1.5 -0.2 0.0025)
                        (cl-tf:axis-angle->quaternion
                         (cl-tf:make-3d-vector 0 0 1) (/ (- pi) 2))))
            ;; (trash2 . ,(cl-tf:make-pose
            ;;             (cl-tf:make-3d-vector -9.74 -7.11 0.0025)
            ;;             (cl-tf:axis-angle->quaternion
            ;;              (cl-tf:make-3d-vector 0 0 1) (/ (- pi) 2))))
            (trash3 . ,(cl-tf:make-pose
                        (cl-tf:make-3d-vector 6.12 -8.63  0.0025)
                        (cl-tf:make-identity-rotation)))
            (trash4 . ,(cl-tf:make-pose
                        (cl-tf:make-3d-vector -3.48 3.41 0.0025)
                        (cl-tf:make-identity-rotation))))))
    (setf *active-goal-transforms*
          (append *active-goal-transforms* ;; (subseq (alexandria:shuffle +rooms-list+) 0 5)
                  trash)))
  ...
  (setf *goal-picked-publisher*
        (roslisp:advertise "goal_picked_up" "std_msgs/String"))
  (sleep 1.0))
...
(defun referee ()
  ...
  (loop until (handler-case
                  (when (lookup-transform *transform-listener*
                                          ...
                                          *frame-id*
                                          ...
                                          :timeout 5)
                    (roslisp:ros-info referee "Robot's /base_footprint found!")
                    T)
                (cl-tf:timeout-error ()
                  (roslisp:ros-warn referee
                                    "Waiting for robot to be published on TF.")
                  NIL)))

  ;; with a tf-buffer running, publish only once
  ...
  (let ((active-goals-tmp (copy-alist *active-goal-transforms*))
    ...
    (loop-at-most-every 0.5
      ...
      ;; lookup all goals->base_footprint to register pickup
      ...
      (when (> treasures-in-trunk 0)
        (let ((transform (lookup-transform *transform-listener*
                                           ...
                                           "goal_depot"
                                           :timeout 5.0)))
          (when (< (v-norm (translation transform)) 1)
            (ros-info depot "Unloading all treasures.")
            (publish-goal-collect "unloading")
  ...
  (subscribe "goal_picked_up"
             ...
             (lambda (msg)
               (roslisp:with-fields (data) msg
                 (roslisp:ros-info goal-sub
                                   "Yeah, the treasure ~a was collected!"
                                   data)))))
